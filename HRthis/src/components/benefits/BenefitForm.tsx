...
export const BenefitForm: React.FC<BenefitFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!title || !description || !coinCost || !category) {
    ...
    const stock = unlimited ? undefined : parseInt(stockLimit);
    if (!unlimited && (isNaN(stock!) || stock! <= 0)) {
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 shadow-sm space-y-4 border-2 border-blue-200">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            value={category}
            onChange={(e) => setCategory(e.target.value as 'WELLNESS' | 'FOOD' | 'TECH' | 'TIME_OFF' | 'OTHER')}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            required
          >
